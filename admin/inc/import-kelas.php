	...
	if (empty($filename) || $filename == "") {
		...
		if (in_array($realtype, $listext)) {
			...
			if ($move) {
				...
				for ($i=2; $i <= $lastRow; $i++) :
					$nama_kelas = anti_inject($worksheet->getCell('B'.$i)->getValue());
					$nama_kelas = addslashes($nama_kelas);
					$paket = addslashes($worksheet->getCell('C'.$i)->getValue());
					$wali_kelas = addslashes($worksheet->getCell('D'.$i)->getValue());

					$ins = insert('tbl_kelas', "id, nama_kelas, paket, wali_kelas", "NULL, '$nama_kelas', '$paket', '$wali_kelas'");
