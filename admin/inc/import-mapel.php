<div class="col-md-12">
	<div class="row">
		<div class="col-md-8">
			<h4>Import Data Mata Pelajaran</h4>
			<hr>
			<br>
			<div class="panel panel-default">
				<div class="panel-body">
					<center>
						<a href="<?=base('admin/download/file/sample_mapel.xlsx');?>" class="btn btn-success">Download Template</a>
						<a href="<?=base('admin/mata-pelajaran');?>" class="btn btn-primary">Kembali</a>
					</center>
					<br>
	...
	if (empty($filename) || $filename == "") {
		...
		if (in_array($realtype, $listext)) {
			...
			if ($move) {
				...
				for ($i=2; $i <= $lastRow; $i++) :
					$kode_mapel = anti_inject($worksheet->getCell('B'.$i)->getValue());
					$nama_mapel = $worksheet->getCell('C'.$i)->getValue();
					$nama_mapel = addslashes($nama_mapel);
					$kelas = anti_inject($worksheet->getCell('D'.$i)->getValue());
					$paket = anti_inject($worksheet->getCell('E'.$i)->getValue());
					$kelompok = anti_inject($worksheet->getCell('F'.$i)->getValue());

					$ins = insert('tbl_mapel', "id, kode_mapel, nama_mapel, kelas, paket, kelompok", "NULL, '$kode_mapel', '$nama_mapel', '$kelas', '$paket', '$kelompok'");

				endfor;

				if ($ins) {
					echo "<script>swal('Yosh!', 'Data mata pelajaran berhasil diimport!', 'success');</script>";
					echo notice(1);
					echo location(base('admin/mata-pelajaran'));
				} else {
					echo "<script>sweetAlert('Oops!', 'Data mata pelajaran Gagal diimport!', 'error');</script>";	
					echo notice(0);
				}
